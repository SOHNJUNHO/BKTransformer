...
class BKTransformerBaseline(nn.Module):
    ...
    def forward(self, obs, output, lambd=None):
        ...
        positions = torch.arange(0, seq_len, dtype=torch.long, device=obs.device)
        position_embeddings = self.pos_emb(positions)

        x = self.drop(token_embeddings + position_embeddings)
        causal_mask = torch.triu(torch.ones(seq_len, seq_len, device=obs.device), diagonal=1).bool()
